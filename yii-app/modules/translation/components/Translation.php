...
class Translation extends Component
{
    ...
    public function getTranslationData()
    {
        ...
        foreach ($module->languages as $lang => $name) {
            if ($module->sourceLanguage && $lang !== $module->sourceLanguage) {
                foreach ($module->mapping as $pathKey => $pathAlias) {
                    $langDir = $this->getLangDir($lang, $pathKey);
    ...
    private function setCategoryFilePath($lang, $category)
    {
        ...
        $this->_categoryFilePath = $this->getLangDir($lang, $category) . DIRECTORY_SEPARATOR . $category . '.php';
    }
    
    private function getLangDir($lang, $category)
    {
        $messagesAlias = null;
        foreach (Yii::$app->getModule('translation')->mapping as $pathKey => $pathAlias) {
            if($category === $pathKey){
                $messagesAlias = $pathAlias;
            }
        }
        $langDir = Yii::getAlias($messagesAlias) . DIRECTORY_SEPARATOR . $lang;
        if (!is_dir($langDir)) {
            mkdir($langDir);
        }
        return $langDir;
